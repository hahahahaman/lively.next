...
(function (exports,lively_lang,escodegen,acorn) {
'use strict';
...
var ComparisonVisitor = function (_Visitor2) {
  ...
  createClass(ComparisonVisitor, [{
    ...
    value: function compareField(field, node1, node2, state) {
      node2 = lively_lang.Path(state.completePath.join('.')).get(node2);
    ...
    value: function accept(node1, node2, state, path) {
      var patternNode = lively_lang.Path(path.join('.')).get(node2);
...
var walk$1 = acorn.walk;
var loose$1 = acorn.loose;
// rk 2016-05-17 FIXME: the current version of acorn.walk doesn't support async
// await. We patch the walker here until it does
if (!walk$1.base.AwaitExpression) {
  walk$1.base.AwaitExpression = function (node, st, c) {
    if (node.argument) c(node.argument, st, 'Expression');
  };
}

// FIXME, don't add to walk object, that's our own stuff!
walk$1.forEachNode = forEachNode;
walk$1.matchNodes = matchNodes;
walk$1.findNodesIncluding = findNodesIncluding;
walk$1.withParentInfo = withParentInfo;
walk$1.copy = copy;
walk$1.findSiblings = findSiblings;

walk$1.findNodeByAstIndex = findNodeByAstIndex;
walk$1.findStatementOfNode = findStatementOfNode;
walk$1.addAstIndex = addAstIndex;
...
function forEachNode(parsed, func, state, options) {
  ...
  var visitors = lively_lang.obj.clone(options.visitors ? options.visitors : walk$1.make(walk$1.visitors.withMemberExpression));
  ...
  walk$1.recursive(parsed, 0, null, visitors);
  return parsed;
}
...
function findNodesIncluding(parsed, pos, test, base) {
  var nodes = [];
  base = base || walk$1.make(walk$1.visitors.withMemberExpression);
  Object.keys(walk$1.base).forEach(function (name) {
    var orig = base[name];
    base[name] = function (node, state, cont) {
  ...
  walk$1.findNodeAround(parsed, pos, test, base);
  return nodes;
}
...
function withParentInfo(parsed, iterator, options) {
  ...
  walk$1.recursive(parsed, { scope: makeScope(), parent: null, propertyInParent: '', depth: 0 }, null, visitors);
...
walk$1.visitors = {
  stopAtFunctions: walk$1.make(_extends({
    'Function': function Function() {/* stop descent */}
  }, walk$1.base)),

  withMemberExpression: walk$1.make({
    MemberExpression: function MemberExpression(node, st, c) {
      c(node.object, st, "Expression");
      c(node.property, st, "Expression");
    }
...
function fuzzyParse(source, options) {
  ...
  try {
    ...
    ast = parse$1(safeSource || source, options);
  ...
  if (!ast) {
    ast = loose$1.parse_dammit(source, options);
...
var methods = {
  ...
  rematchAstWithSource: function rematchAstWithSource(parsed, source, addLocations, subTreePath) {
    ...
    var parsed2 = parse$1(source, addLocations ? { locations: true } : undefined),
        visitor = new Visitor();
...
function topLevelDeclsAndRefs(parsed, options) {
  ...
  if (typeof parsed === "string") parsed = parse$1(parsed, options);
...
function findNodesIncludingLines(parsed, code, lines, options) {
  ...
  parsed = parsed && parsed.loc ? parsed : parse$1(code, { locations: true });
  return withMozillaAstDo(parsed, [], function (next, node, found) {
...
function nodesAt$1(pos, ast) {
  ast = typeof ast === 'string' ? parse$1(ast) : ast;
...
function oneDeclaratorPerVarDecl(astOrSource) {
  ...
  var parsed = (typeof astOrSource === "undefined" ? "undefined" : _typeof(astOrSource)) === 'object' ? astOrSource : parse$1(astOrSource),
      source = typeof astOrSource === 'string' ? astOrSource : parsed.source || _node2string(parsed),
      scope = scopes(parsed),
...
function returnLastStatement(source, opts) {
  ...
  var parsed = parse$1(source, opts),
...
function wrapInStartEndCall(parsed, options) {
  ...
  if (typeof parsed === "string") parsed = parse$1(parsed);
  options = options || {};
...
function extractComments(astOrCode, optCode) {
  var parsed = typeof astOrCode === "string" ? parse$1(astOrCode, { withComments: true }) : astOrCode,
...
function findDecls(parsed, options) {
  ...
  if (typeof parsed === "string") parsed = parse$1(parsed, { addSource: true });
